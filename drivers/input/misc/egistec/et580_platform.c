...
static void egis_reset_high_low(struct egis_data *egis, int hi_low)
{
	pr_debug("[egis] %s  hi_low = %d \n", __func__, hi_low);
	if (hi_low) {
#if SDM855_EGIS
...
static long egis_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case FP_RESET_SET:
		...
		egis_reset_high_low(egis, data.int_mode);
		pr_debug("[egis] %s: egis_reset_high_low = %d\n", __func__, data.int_mode);
		break;
